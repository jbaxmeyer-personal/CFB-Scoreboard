...
/** One FBS conference, and who plays in it. */
 ...
 *
 * Built from the core API rather than read off the scoreboard, because the
 * scoreboard payload is not known to carry a conference for each team and
 * guessing at its shape is exactly what cost three rounds earlier. This
 * walks a path already proven to work here: group 80's children are the
 * conferences, and each conference's `teams` collection is the same
 * endpoint shape fetchFbsTeamIds already reads.
 *
 * Costs one request for the conference list plus one per conference, so it
 * is fetched lazily — only when the conference filter is actually opened —
 * and cached for the session. Membership changes once a year, not once a
 * Saturday.
 */
export async function fetchConferences(year: number): Promise<Conference[]> {
  const children = await getCoreJson<{ items?: (EspnCoreRef & { id?: string })[] }>(
    `${CORE_URL}/seasons/${year}/types/2/groups/${FBS_GROUP}/children?limit=100`,
    'core FBS conferences',
  )

  const refs = (children.items ?? []).map((item) => item.$ref).filter((ref): ref is string => Boolean(ref))
  const conferences = await Promise.all(
    refs.map(async (ref) => {
      try {
        const group = await getCoreJson<{
          id?: string
          name?: string
          shortName?: string
          abbreviation?: string
        ...
        const id = group.id ?? idFromRef(ref, 'groups')
        const name = group.name ?? group.shortName ?? group.abbreviation
        if (!id || !name) return null

        const teamIds = new Set<string>()
        if (group.teams?.$ref) {
          const teams = await getCoreJson<{ items?: (EspnCoreRef & { id?: string })[] }>(
            `${group.teams.$ref}${group.teams.$ref.includes('?') ? '&' : '?'}limit=200`,
            'core conference teams',
          )
          for (const item of teams.items ?? []) {
            const teamId = item.id ?? idFromRef(item.$ref, 'teams')
            if (teamId) teamIds.add(teamId)
          }
        }
        // A conference we can name but whose members we can't resolve would
        // be an option that filters everything away, which reads as a bug.
        if (teamIds.size === 0) return null
        return { id, name, shortName: group.shortName ?? group.abbreviation ?? name, teamIds }
      } catch {
        // One unreadable conference shouldn't cost the whole list.
        return null
      }
    }),
  )
